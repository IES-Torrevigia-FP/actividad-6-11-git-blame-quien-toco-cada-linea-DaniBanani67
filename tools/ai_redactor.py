...
Extracto Reflexion 6.11 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.11) se califica con rubrica: Uso correcto de git blame en archivo completo y en rangos de lineas combinandolo con git show/log (max 4), Evidencias claras en los checkpoints (hashes identificados, contexto de commits, rangos) (max 3), Reflexion 6.11 (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece blame_checks.cp1_historial_blame.ok en true, reconoce explicitamente el historial preparado con commits de sumar/restar/multiplicar/bug.
- Si en JSON aparece blame_checks.cp2_blame_completo.ok en true, reconoce explicitamente el uso de git blame sobre el archivo completo con explicacion.
- Si en JSON aparece blame_checks.cp3_investigacion_bug.ok en true, reconoce explicitamente la investigacion del bug con git blame + git show.
- Si en JSON aparece blame_checks.cp4_blame_rango.ok en true, reconoce explicitamente el uso de git blame -L para limitar el analisis a un rango de lineas.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-11.md'))
