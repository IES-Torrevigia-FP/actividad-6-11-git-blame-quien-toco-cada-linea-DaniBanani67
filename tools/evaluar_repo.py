...
def analyze_commits(root: Path) -> dict:
    ...
    for line in log:
        ...
        imperative = any(
            ...
            for p in ['add', 'create', 'anade', 'agrega', 'implement', 'refactor', 'remove', 'elimina',
                      'document', 'docs', 'feat', 'fix', 'chore', 'merge', 'rebase', 'revert',
                      'resuelve', 'commit', 'inicial', 'modifica', 'introduce']
        )
        score = 0
        if msg_len >= 12:
            score += 0.4
...
def analyze_blame_activity(root: Path) -> dict:
    cp1_path = root / 'cp1-historial-blame.txt'
    cp2_path = root / 'cp2-blame-completo.txt'
    cp3_path = root / 'cp3-investigacion-bug.txt'
    cp4_path = root / 'cp4-blame-rango.txt'

    # Buscar el archivo de calculadora con cualquier extension
    calc_path = None
    for ext in ('calculadora.js', 'calculadora.py', 'calculadora.ts'):
        candidate = root / ext
        if candidate.exists():
            calc_path = candidate
            break
    ...
    # cp1: historial con git log --oneline; al menos commits de sumar/restar/multiplicar/bug
    cp1_log_ok = contains_any(cp1, ['git log --oneline', 'sumar', 'restar', 'multiplicar', 'bug', 'todo'])

    # cp2: blame completo con lineas de restar/todo y explicacion
    cp2_blame_lines_ok = (
        contains_any(cp2, ['git blame', 'blame calculadora'])
        or contains_any(cp2, ['restar', 'todo', 'a - b', 'bug'])
    )
    cp2_explain_ok = contains_any(
        cp2, ['hash', 'autor', 'commit', 'fecha', 'informacion', 'información', 'linea', 'línea']
    )

    # cp3: investigacion del bug con blame + show
    cp3_blame_line_ok = contains_any(cp3, ['restar', 'bug', 'blame', 'a - b - 1'])
    cp3_show_ok = contains_any(cp3, ['git show', 'show '])
    cp3_explain_ok = contains_any(
        cp3, ['cuando', 'cuándo', 'introdujo', 'introduc', 'commit', 'aprend', 'bug']
    )

    # cp4: blame con rango -L
    cp4_rango_ok = (
        contains_any(cp4, ['git blame -l', 'git blame -L', 'blame -l', 'blame -L', '-l ', '-L '])
        and contains_any(cp4, ['restar', 'calculadora'])
    )
    cp4_explain_ok = contains_any(
        cp4, ['util', 'útil', 'bloque', 'rango', 'funcion', 'función', 'centrar', 'concret']
    )

    return {
        'cp1_historial_blame': {
            'ok': cp1_log_ok,
            'source': 'cp1-historial-blame.txt' if cp1_path.exists() else 'missing',
        },
        'cp2_blame_completo': {
            'ok': cp2_blame_lines_ok and cp2_explain_ok,
            'source': 'cp2-blame-completo.txt' if cp2_path.exists() else 'missing',
        },
        'cp3_investigacion_bug': {
            'ok': cp3_blame_line_ok and cp3_show_ok and cp3_explain_ok,
            'source': 'cp3-investigacion-bug.txt' if cp3_path.exists() else 'missing',
        },
        'cp4_blame_rango': {
            'ok': cp4_rango_ok and cp4_explain_ok,
            'source': 'cp4-blame-rango.txt' if cp4_path.exists() else 'missing',
        },
        'calculadora_presente': {
            'exists': calc_path is not None,
            'path': str(calc_path.name) if calc_path else None,
        },
    }
...
def main():
    ...
    ap.add_argument(
        '--required',
        default='cp1-historial-blame.txt,cp2-blame-completo.txt,cp3-investigacion-bug.txt,cp4-blame-rango.txt,reflexion-6-11.md',
    )
    ...
    blame_info = analyze_blame_activity(root)
    ...
    reflex_path = root / 'reflexion-6-11.md'
    ...
    evidencia_candidates = [
        'cp1', 'cp2', 'cp3', 'cp4',
        'blame', 'calculadora', 'bug', 'show', 'rango', 'reflexion',
    ]
    ...
    for cand in evidencia_candidates:
        for p in root.rglob(f'*{cand}*'):
            if outdir.name in p.parts or '.git' in p.parts:
                continue
            if p.is_file() and p.suffix.lower() in ('.md', '.png', '.jpg', '.jpeg', '.txt', '.js', '.py', '.ts', ''):
    ...
    # Criterio 1 (max 4): uso correcto de git blame (archivo completo, rangos, blame + show/log)
    checks_blame = [
        blame_info['cp1_historial_blame']['ok'],
        blame_info['cp2_blame_completo']['ok'],
        blame_info['cp3_investigacion_bug']['ok'],
        blame_info['cp4_blame_rango']['ok'],
        blame_info['calculadora_presente']['exists'],
    ]
    blame_hits = sum(1 for x in checks_blame if x)
    if blame_hits >= 5:
        s_blame = 4
    elif blame_hits >= 4:
        s_blame = 3
    ...
    # Criterio 2 (max 3): evidencias claras (hashes, contexto, rangos)
    evidencia_checks = [
        blame_info['cp2_blame_completo']['ok'],
        blame_info['cp3_investigacion_bug']['ok'],
        blame_info['cp4_blame_rango']['ok'],
    ]
    evidencia_hits = sum(1 for x in evidencia_checks if x)
    if evidencia_hits >= 3:
        s_evidencias = 3
    elif evidencia_hits >= 2:
        s_evidencias = 2
    elif evidencia_hits >= 1:
        s_evidencias = 1
    else:
        s_evidencias = 0
    ...
    total = s_blame + s_evidencias + s_reflex
    ...
    resumen = {
        ...
        'scores': {
            'git_blame_uso (max_4)': s_blame,
            'evidencias_checkpoints (max_3)': s_evidencias,
    ...
    with (outdir / 'metricas.csv').open('w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(['git_blame_uso (max 4)', s_blame])
        w.writerow(['evidencias_checkpoints (max 3)', s_evidencias])
    ...
    md.append('|---|---:|')
    md.append(f"| Uso correcto de git blame (archivo completo, -L, blame + show/log) | {s_blame}/4 |")
    md.append(f"| Evidencias claras en checkpoints (hashes, contexto, rangos) | {s_evidencias}/3 |")
    md.append(f"| Reflexion 6.11 | {s_reflex}/3 |")
    ...
    md.append('## Checks de git blame')
    md.append(f"- Historial inicial calculadora (cp1): {'si' if blame_info['cp1_historial_blame']['ok'] else 'no'}  ")
    md.append(f"- Blame completo con explicacion (cp2): {'si' if blame_info['cp2_blame_completo']['ok'] else 'no'}  ")
    md.append(f"- Investigacion bug con blame + show (cp3): {'si' if blame_info['cp3_investigacion_bug']['ok'] else 'no'}  ")
    md.append(f"- Blame con rango -L y explicacion (cp4): {'si' if blame_info['cp4_blame_rango']['ok'] else 'no'}  ")
    md.append(f"- Archivo calculadora presente: {'si' if blame_info['calculadora_presente']['exists'] else 'no'}  ")
    md.append('')
